 ...
 *
 * Updated: 2026-01-04 for v1.17.3 (E2E CI Pipeline Stabilization - GitHub Actions integration, filterCIErrors helper, browser-specific handling)
 */
...
// =============================================================================
// TECH STACK KNOWLEDGE (v1.17.3 - updated 2026-01-04)
// =============================================================================
...
export const WEBSITE_FEATURES = {
  ...
  navigation: {
    ...
    footer: {
      columns: ["Features (6 links)", "Documentation (7 links)", "Resources (6 links)", "Project (7 links)", "Legal (4 links)"],
      layout: "Flex + Grid hybrid: fixed-width brand section (lg:w-64) + 5-column unified grid (lg:grid-cols-5)",
      bottomBar: ["Logo", "Copyright", "Author", "Version", "Language selector", "Sound toggle", "Theme toggle"],
    },
    mobileBottomNav: {
...
export const PROJECT_KNOWLEDGE_CHUNKS = [
  ...
  },
  {
    ...
    content: `Claude Insider v1.17.0 is a comprehensive chat system rewrite following Matrix SDK patterns. Phase 1 - LRU Cache System: Generic LRUCache<K,V> class with TTL and auto-cleanup. Specialized caches: MessageCache (5min TTL, 1000 items), ConversationCache (2min, 100), PresenceCache (30s, 500), UserProfileCache (10min, 200). Phase 2 - Delivery Status Tracking: DeliveryTracker class with sending→sent→delivered→read progression. DeliveryStatusIndicator component with visual checkmarks. Per-recipient tracking in dm_delivery_receipts table. Phase 3 - Rich Media Preview: LinkUnfurler service with URL extraction. Server-side /api/chat/unfurl endpoint bypasses CORS. Open Graph metadata parsing. LinkPreviewCard with YouTube/Vimeo detection. 7-day cache in link_previews table. Phase 4 - Message Pinning: Server actions (pinMessage, unpinMessage, getPinnedMessages). PinnedMessagesPanel slide-out UI with jump-to-message. Admin/owner-only permissions with optional pin notes. Phase 5 - Chat Performance Optimizations: Batched realtime updates (50ms window, max 20 items). Subscription pooling with reference counting. RequestDeduplicator for concurrent request coalescing. Optimized SQL functions (get_messages_with_context, get_users_presence, get_conversations_with_context). Phase 6 - E2EE Default for DMs: setupDMEncryption() and upgradeDMToE2EE() utilities. Automatic encryption setup for new DMs. Device key sync via dm_device_keys table. Graceful fallback when E2EE unavailable. Note: Voice messages and file attachments were originally added in v1.17.0 but later removed in v1.18.0.`,
    url: "/changelog",
    category: "Project",
