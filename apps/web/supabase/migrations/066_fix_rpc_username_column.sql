-- Migration: Fix RPC Functions - Correct username column reference
-- Description: The profiles table does NOT have a username column - it's only on the user table
--
-- Schema reality:
-- - profiles: display_name, avatar_url, bio (NO username)
...
BEGIN
  ...
  WITH conversation_participants AS (
    SELECT
      dp.conversation_id,
      dp.user_id,
      -- display_name from profiles, fallback to name from user
      COALESCE(p.display_name, u.name, 'Unknown') as name,
      -- username is ONLY on user table, NOT on profiles
      u.username as username,
      -- avatar_url from profiles, fallback to image from user
...
-- Comment on fix
COMMENT ON FUNCTION get_conversations_optimized IS 'Returns conversations - FIXED: username is only on user table, not profiles';
COMMENT ON FUNCTION get_messages_paginated IS 'Returns paginated messages - FIXED: username is only on user table, not profiles';
