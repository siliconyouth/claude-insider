...
/**
 * ResourceAIOperations - Payload CMS Custom Field
 *
 * Displays AI operation controls for resources:
 * - CLI commands for resource enhancement (run in Claude Code)
 * - Queue operation for tracking
 * - Status of pending/completed operations
 *
 * Architecture: AI operations run in Claude Code CLI to use subscription credits
 */
...
export default function ResourceAIOperations() {
  ...
  // Generate CLI commands based on resource ID
  const enhanceCommand = documentId ? `node scripts/enhance-resources.mjs --id="${documentId}"` : '';
  const analyzeCommand = documentId ? `node scripts/analyze-relationships.mjs --resource="${documentId}"` : '';

  // Fetch pending operations for this resource
  useEffect(() => {
    if (!documentId) return;

    const fetchPendingOps = async () => {
      try {
        const response = await fetch(`/api/dashboard/ai-queue?target_type=resource&target_id=${documentId}`);
        if (response.ok) {
    ...
    return (
      <div style={styles.container}>
        <p style={styles.muted}>
          Save this resource first to enable AI operations.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Enhance Resource Command */}
      <div style={styles.commandSection}>
        <label style={styles.commandLabel}>Enhance Resource</label>
        <p style={styles.commandHelp}>
          Scrapes the resource URL and generates summary, features, pros/cons, and use cases.
        </p>
        <div style={styles.commandRow}>
          <code style={styles.commandCode}>{enhanceCommand}</code>
          <button
            onClick={() => handleCopyCommand(enhanceCommand)}
            style={styles.copyButton}
            title="Copy to clipboard"
          >
            {copiedCommand === enhanceCommand ? <CheckIcon /> : <CopyIcon />}
          </button>
        </div>
        <button
          onClick={() => handleQueueOperation('enhance_resource')}
          disabled={isQueueing}
      ...
      <div style={styles.commandSection}>
        ...
        <p style={styles.commandHelp}>
          Discovers connections to documentation and other resources.
        </p>
        <div style={styles.commandRow}>
