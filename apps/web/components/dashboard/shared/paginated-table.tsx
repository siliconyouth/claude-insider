    ...
    return (
      <div className={cn("space-y-3", className)}>
        {[1, 2, 3, 4, 5].map((i) => (
          <div
            key={i}
            className="h-16 rounded-lg ui-bg-skeleton animate-pulse"
          />
        ))}
      </div>
    );
            ...
            <tr className="border-b ui-border">
              {columns.map((column) => (
                <th
                  ...
                  className={cn(
                    "px-4 py-3 text-left text-xs font-medium ui-text-secondary uppercase tracking-wider",
            ...
            {items.map((item) => {
              ...
              return (
                <tr
                  ...
                  className={cn(
                    "transition-colors",
                    onRowClick && "cursor-pointer ui-hover-row",
                    rowClass
                  )}
                >
                  {columns.map((column) => (
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          className={cn(
            ...
            canGoPrev
              ? "ui-text-body ui-btn-ghost"
              : "ui-text-muted cursor-not-allowed"
          )}
        >
          Previous
        </button>
        ...
        <div className="flex items-center gap-1 mx-2">
          {getPageNumbers().map((pageNum, idx) =>
            pageNum === "..." ? (
              <span key={`ellipsis-${idx}`} className="px-2 ui-text-muted">
                ...
              </span>
            ) : (
              <button
    ...
    return (
      <div className={cn("grid gap-4", gridCols[columns], className)}>
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-32 rounded-xl ui-bg-skeleton animate-pulse"
          />
        ))}
      </div>
    );
